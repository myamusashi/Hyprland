...
static int hlPlugin(lua_State* L) {
    auto*            mgr = static_cast<CConfigManager*>(lua_touserdata(L, lua_upvalueindex(1)));
    ...
    if (pathErr.errorCode != PARSE_ERROR_OK)
        return luaL_error(L, "%s", std::format("hl.plugin: first argument (path) must be a string: {}", pathErr.message).c_str());

    const auto& path = pathParser.parsed();

    if (path.empty())
        return luaL_error(L, "hl.plugin: path must not be empty");
...
static int hlPermission(lua_State* L) {
    ...
    if (lua_istable(L, 1)) {
        auto b = tableOptStr(L, 1, "binary");
        if (!b)
            b = tableOptStr(L, 1, "target");
        auto t = tableOptStr(L, 1, "type");
        auto m = tableOptStr(L, 1, "mode");
...
void Bindings::registerBindings(lua_State* L, CConfigManager* mgr) {
    ...
    // hl.plugin
    lua_pushlightuserdata(L, mgr);
    lua_pushcclosure(L, hlPlugin, 1);
    lua_setfield(L, -2, "plugin");

    // hl.permission
    lua_pushlightuserdata(L, mgr);
    lua_pushcclosure(L, hlPermission, 1);
    ...
    lua_pushcfunction(L, hlGetActiveWindow);
    lua_setfield(L, -2, "get_active_window");
