...
def parse_config_values(root: Path) -> dict[str, str]:
    cfg = root / "src/config/values/ConfigValues.hpp"
    ...
    for vtype, key in pattern.findall(source):
        out[key.replace(":", ".")] = type_map.get(vtype, "any")
...
def generate_stub(root: Path) -> str:
    ...
    api_signatures: dict[str, str] = {
        ...
        "hl.notification.create": "fun(text: string, opts?: HL.NotificationOptions): HL.Notification",
        "hl.get_notifications": "fun(): HL.Notification[]",
    }
