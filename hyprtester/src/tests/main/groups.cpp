...
static bool test() {
    ...
    Tests::dispatchLua("hl.workspace(\"name:groups\")");
    ...
    OK(Tests::dispatchLua("hl.group.toggle()"));
    OK(Tests::evalLua("hl.config({ [\"group.groupbar.enabled\"] = true })"));
    ...
    OK(Tests::evalLua("hl.config({ [\"group.groupbar.enabled\"] = false })"));
    ...
    OK(Tests::dispatchLua("hl.group.next()"));
    ...
    Tests::dispatchLua("hl.group.next()");
    ...
    try {
        ...
        OK(Tests::dispatchLua("hl.group.move_window({ forward = true })"));
    ...
    try {
        auto str              = getFromSocket("/activewindow");
        auto activeBeforeMove = std::stoull(str.substr(7, str.find(" -> ") - 7), nullptr, 16);
        OK(Tests::dispatchLua("hl.group.move_window({ forward = false })"));
        str                  = getFromSocket("/activewindow");
        auto activeAfterMove = std::stoull(str.substr(7, str.find(" -> ") - 7), nullptr, 16);
        EXPECT(activeAfterMove, activeBeforeMove);
    } catch (...) {
    ...
    OK(Tests::evalLua("hl.config({ [\"group.auto_group\"] = false })"));
    ...
    OK(Tests::dispatchLua("hl.focus({ direction = \"l\" })"));
    OK(Tests::dispatchLua("hl.group.active({ index = 1 })"));
    OK(Tests::evalLua("hl.config({ [\"group.auto_group\"] = true })"));
    OK(Tests::evalLua("hl.config({ [\"group.insert_after_current\"] = false })"));
    ...
    OK(Tests::dispatchLua("hl.group.active({ index = 3 })"));
    ...
    OK(Tests::evalLua("hl.config({ [\"group.groupbar.enabled\"] = false })"));
    {
        ...
        OK(Tests::dispatchLua("hl.window.move({ into_group = \"l\" })"));

        // move active window out of group downward
        NLog::log("{}Test movewindoworgroup d", Colors::YELLOW);
        OK(Tests::dispatchLua("hl.window.move({ direction = \"d\", group_aware = true })"));
    ...
    OK(Tests::evalLua("hl.window_rule({ name = \"kitty-tiled\", match = { class = \"kitty_tiled\" }, tile = true })"));
    ...
    OK(Tests::evalLua("hl.window_rule({ name = \"kitty-floated\", match = { class = \"kitty_floated\" }, float = true })"));
    ...
    // Test locked groups
    {
        ...
        OK(Tests::dispatchLua("hl.group.toggle()"));
        OK(Tests::dispatchLua(std::format("hl.focus({{ window = {} }})", Tests::luaQuote(std::format("pid:{}", lockedWin->pid())))));
        OK(Tests::dispatchLua("hl.group.lock_active({ action = \"enable\" })"));
    ...
    {
        OK(Tests::evalLua("hl.window_rule({ name = \"locked-im\", match = { class = \"^locked|invade$\" }, group = \"set always lock invade\" })"));
